...
if($num == 0){
    $sql="INSERT INTO allmember (Username, Pwd, Firstname, Surname, RoleName) VALUES ('$Username' ,'$Pwd' ,'$Firstname' ,'$Surname' ,'$RoleName') ";
    $result=mysqli_query($conn,$sql);
    echo "<script>alert('บันทึกข้อมูลเรียบร้อย');</script>";
    echo "<script>window.location='A-allmember-list.php';</script>";
}else{
    echo "<script>alert('ไม่สามารถบันทึกข้อมูลได้');</script>";
    echo "<script>window.location='A-allmember-add.php';</script>";
